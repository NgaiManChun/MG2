...
namespace MG {
	...
	bool VertexBoneWeightDivision::ReserveMeta(unsigned int newCapcity)
	{
	...
	bool VertexBoneWeightDivision::ReserveData(unsigned int newCapcity)
	{
		if (newCapcity < s_DataSize) return false;

		ID3D11Buffer* newBuffer = Renderer::CreateStructuredBuffer(sizeof(VERTEX_BONE_WEIGHT), newCapcity, nullptr);
		if (!newBuffer) return false;
	...
	void VertexBoneWeightDivision::Init()
	{
		if (!s_PadCS) {
	...
	VertexBoneWeightDivision VertexBoneWeightDivision::Create(unsigned int count, const VERTEX_BONE_WEIGHT* data) {
		...
		// update data
		if (data) {
			D3D11_BOX box = Renderer::GetRangeBox(sizeof(VERTEX_BONE_WEIGHT) * meta.offset, sizeof(VERTEX_BONE_WEIGHT) * (meta.offset + meta.count));
