...
namespace MG {
	...
	bool VertexIndexDivision::ReserveData(unsigned int newCapcity)
	{
		if (newCapcity < s_DataCount) return false;

		ID3D11Buffer* newBuffer = Renderer::CreateVertexBuffer(sizeof(unsigned int) * newCapcity);
	...
	{
		ID3D11Buffer* copyBuffer = Renderer::CreateStructuredBuffer(sizeof(unsigned int), s_DataCount);
		...
		ID3D11Buffer* resultBuffer = Renderer::CreateStructuredBuffer(
			sizeof(unsigned int), s_DataCount,
			nullptr,
	...
	void VertexIndexDivision::Init()
	{
		if (!s_PadCS) {
			s_PadCS = Renderer::LoadComputeShader("complied_shader\\padVertexIndexCS.cso");
		}
	}
